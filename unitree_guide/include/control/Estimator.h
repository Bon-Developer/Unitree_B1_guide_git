...
/**********************************************************************
Release v0.1(250313)
설명: 보행 로봇의 상태를 추정하는 클래스 정의, 발 위치, 속도, 자세 정보 등을 센서 데이터와 칼만 필터 유사 방법을 사용하여 추정
함수
- Estimator: 로봇 모델, 센서 데이터, 다리 접촉 정보 등 받아와 내부 상태를 초기화, dt= 샘플링 간격
- getPosition: 로봇의 추정된 위치 반환, 센서 데이터와 발 위치 정보를 조합하여 로봇 중심(CoM) 위치 추정
- getVelocity: 로봇의 추정된 속도 반환
- getFootPos: 특정 다리의 현재 위치를 반환
- getFeetVel: 모든 다리의 속도 반환
- getPosFeet2BGlobal: 다리의 위치를 몸체 좌표계에서 전역 좌표계로 변환
- _initSystem: 초기 상태 설정 함수
***********************************************************************/
