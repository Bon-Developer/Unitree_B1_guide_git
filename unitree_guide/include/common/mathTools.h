...
/**********************************************************************
Release v0.1(250313)
설명: 수학적 계산과 유틸리티 및 함수 템플릿 형식 제공하는 라이브러리 정의
함수
- max, min: 최대 / 최소 값 반환
- saturation: 주어진 범위 값 제한
- kiilZeroOffset: 특정 임계값 내의 작은 값을 0으로 설정하여 데이터 졍규화
- invNormalize: 역정규화, 데이터를 원하는 범위로 스케일 조정할 때 사용
- windowFunc: 신호 처리에 사용되며 데이터의 범위 및 비율 조정
- updateAverage, updateCovariance: 데이터 평균과 공분산을 점진적으로 업데이터
- updateAvgCov: 평균과 공분산을 동시에 업데이트, 공분산 업데이트 후 평균 업데이트
- rotx, roty, rotz: 각 축에 대한 회전 행렬
- skew: 벡터를 스큐 행렬로 변환
- rpyToRotMat: 롤, 피치, 요를 받아와 회전 행렬을 생성
- quatToRotMat: 쿼터니언을 행렬로 변환
- rotMatToRPY, rotMatToExp: 회전 행렬을 롤, 피치, 요 또는 지수 맵으로 변환
- homoMatrix: 위치 벡터와 회전 행렬 또는 쿼터니언을 사용하여 동차 변홚 행렬 생성
- homoMatrixInverse: 동차 변환 행령의 역행렬 계산
- homoVec, noHomoVec 함수: 3차원 벡터에 동차 좌표를 추가하거나 제거
- AvgCov class: 데이터의 평균과 공분산을 계산하고 관리
***********************************************************************/
